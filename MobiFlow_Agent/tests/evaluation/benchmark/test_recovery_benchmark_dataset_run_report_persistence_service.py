...
from mobiflow_agent.evaluation.benchmark.run_report import (
    RecoveryBenchmarkDatasetRunReportCatalog,
    RecoveryBenchmarkDatasetRunReportDocument,
    RecoveryBenchmarkDatasetRunReportDocumentSchemaVersion,
)
from mobiflow_agent.evaluation.benchmark.run_report import (
...
def _eval_case(
    *,
    case_id: str,
    expected_verdict_status: VerificationStatus = VerificationStatus.VERIFIED_SUCCESS,
    actual_verdict_status: VerificationStatus = VerificationStatus.VERIFIED_SUCCESS,
):
    execution = _execution_response()
    harness_response = _harness_response(verdict_status=actual_verdict_status)
    return ReplayEvalService().build_eval_case(
...
def _dataset(case_id: str, *, matched: bool = True):
    ...
    suite = RecoveryBenchmarkService().build_suite(
...
def _run_report(case_id: str, *, matched: bool = True):
    return RecoveryBenchmarkDatasetRunnerService().run_dataset(_dataset(case_id, matched=matched))
...
def test_save_report_writes_json_document(artifact_tmp_path) -> None:
    service = RecoveryBenchmarkDatasetRunReportPersistenceService()
    report = _run_report("eval:save")
    tmp_path = _test_dir(artifact_tmp_path, "save")
    output_path = tmp_path / "reports" / "alpha.json"

    entry = service.save_report(report=report, output_path=str(output_path))

    assert output_path.exists()
    assert entry.dataset_id == report.dataset_id
    payload = json.loads(output_path.read_text(encoding="utf-8"))
    assert payload["schema_version"] == RecoveryBenchmarkDatasetRunReportDocumentSchemaVersion.V1.value
    assert payload["report"]["dataset_id"] == report.dataset_id


def test_load_report_restores_report_from_json_file(artifact_tmp_path) -> None:
    service = RecoveryBenchmarkDatasetRunReportPersistenceService()
    report = _run_report("eval:load")
    tmp_path = _test_dir(artifact_tmp_path, "load")
    output_path = tmp_path / "report.json"
    service.save_report(report=report, output_path=str(output_path))

    restored = service.load_report(str(output_path))

    assert restored.dataset_id == report.dataset_id
    assert restored.dataset_name == report.dataset_name
    assert restored.result.total_cases == report.result.total_cases


def test_run_report_document_supports_roundtrip() -> None:
    document = RecoveryBenchmarkDatasetRunReportDocument(report=_run_report("eval:roundtrip"))

    restored = RecoveryBenchmarkDatasetRunReportDocument.model_validate(document.model_dump(mode="python"))

    assert restored.schema_version == RecoveryBenchmarkDatasetRunReportDocumentSchemaVersion.V1
    assert restored.report.dataset_id == document.report.dataset_id


def test_save_report_creates_parent_directory(artifact_tmp_path) -> None:
    service = RecoveryBenchmarkDatasetRunReportPersistenceService()
    tmp_path = _test_dir(artifact_tmp_path, "mkdir")
    output_path = tmp_path / "nested" / "reports" / "report.json"

    service.save_report(report=_run_report("eval:mkdir"), output_path=str(output_path))

    assert output_path.parent.exists()
    assert output_path.exists()


def test_load_report_rejects_unsupported_schema_version(artifact_tmp_path) -> None:
    ...
    path.write_text(
        json.dumps(
            {
                "schema_version": "v999",
                "report": _run_report("eval:bad-schema").model_dump(mode="json"),
            }
        ),
        encoding="utf-8",
    )

    with pytest.raises(ValueError, match="Invalid benchmark dataset run report document schema"):
        RecoveryBenchmarkDatasetRunReportPersistenceService().load_report(str(path))


def test_load_report_rejects_invalid_json(artifact_tmp_path) -> None:
    tmp_path = _test_dir(artifact_tmp_path, "bad-json")
    path = tmp_path / "invalid.json"
    path.write_text("{not-json", encoding="utf-8")

    with pytest.raises(ValueError, match="Invalid benchmark dataset run report JSON document"):
        RecoveryBenchmarkDatasetRunReportPersistenceService().load_report(str(path))


def test_save_to_catalog_writes_dataset_id_named_file(artifact_tmp_path) -> None:
    service = RecoveryBenchmarkDatasetRunReportPersistenceService()
    report = _run_report("eval:catalog")
    tmp_path = _test_dir(artifact_tmp_path, "save-catalog")

    entry = service.save_to_catalog(report=report, catalog_dir=str(tmp_path))

    expected_path = tmp_path / f"{quote(report.dataset_id, safe='-_')}.json"
...
def test_list_catalog_returns_entries_sorted_by_dataset_id(artifact_tmp_path) -> None:
    ...
    first = _run_report("eval:b").model_copy(update={"dataset_id": "benchmark-dataset:b"})
    second = _run_report("eval:a").model_copy(update={"dataset_id": "benchmark-dataset:a"})
    service.save_to_catalog(report=first, catalog_dir=str(tmp_path))
    service.save_to_catalog(report=second, catalog_dir=str(tmp_path))

    catalog = service.list_catalog(str(tmp_path))

    assert isinstance(catalog, RecoveryBenchmarkDatasetRunReportCatalog)
...
def test_load_from_catalog_reads_named_report(artifact_tmp_path) -> None:
    service = RecoveryBenchmarkDatasetRunReportPersistenceService()
    report = _run_report("eval:load-catalog")
    tmp_path = _test_dir(artifact_tmp_path, "load-catalog")
    service.save_to_catalog(report=report, catalog_dir=str(tmp_path))

    restored = service.load_from_catalog(catalog_dir=str(tmp_path), dataset_id=report.dataset_id)

    assert restored.dataset_id == report.dataset_id
    assert restored.result.match_rate == report.result.match_rate


def test_list_catalog_rejects_invalid_report_document(artifact_tmp_path) -> None:
    tmp_path = _test_dir(artifact_tmp_path, "invalid-catalog")
    invalid_path = tmp_path / "broken.json"
    invalid_path.write_text(json.dumps({"schema_version": "v1", "report": {"oops": True}}), encoding="utf-8")

    with pytest.raises(ValueError, match="Invalid benchmark dataset run report document schema"):
        RecoveryBenchmarkDatasetRunReportPersistenceService().list_catalog(str(tmp_path))


def test_persistence_service_is_pure_and_does_not_rerun_dataset_runner(artifact_tmp_path) -> None:
    service = RecoveryBenchmarkDatasetRunReportPersistenceService()
    report = _run_report("eval:pure", matched=False)
    tmp_path = _test_dir(artifact_tmp_path, "pure")
    entry = service.save_to_catalog(report=report, catalog_dir=str(tmp_path))

    catalog = service.list_catalog(str(tmp_path))
    restored = service.load_report(entry.path)

    assert catalog.entries[0].dataset_id == report.dataset_id
    assert catalog.entries[0].mismatched_cases == 1
    assert restored.dataset_id == report.dataset_id
    assert restored.result.mismatched_cases == report.result.mismatched_cases
...
def test_list_catalog_missing_directory_raises_file_not_found(artifact_tmp_path) -> None:
    ...
    with pytest.raises(FileNotFoundError):
        RecoveryBenchmarkDatasetRunReportPersistenceService().list_catalog(str(missing_dir))
