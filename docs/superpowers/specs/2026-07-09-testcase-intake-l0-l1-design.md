...
3. **Semantics caveat (must be in tests):** `NOT_EXISTS` matches when the field_path resolves to nothing. This means it also "matches" when the fact_id itself is missing/unobserved — i.e. absence-of-evidence reads as absence-of-element. The synthesis prompt (§5) MUST anchor `NOT_EXISTS` predicates to a `fact_id` that is reliably present (e.g. the screen snapshot), so "no such node in a screen we DID observe" is what's tested, not "we saw nothing at all."
